...
productSchema.methods.calculateUniquenessScore = function() {
  ...
  const uniqueTags = ['limited-edition', 'handmade', 'custom', 'vintage', 'rare', 'one-of-a-kind'];
  const matchingUniqueTags = this.tags.filter(tag => uniqueTags.includes(tag));
  ...
  if (exclusiveBrands.some(brand => this.brand?.toLowerCase().includes(brand))) {
    score += 15;
  }
